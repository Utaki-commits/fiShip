...
export default function DashboardBookingsPage() {
  ...
  const renderBookingCard = (b: Booking, showDate = false) => (
    <div key={b.id} style={{ background: 'var(--surface)', border: b.status === 'confirmed' && !b.contacted ? '1px solid var(--border)' : '1px solid var(--border)', borderLeft: b.status === 'confirmed' && !b.contacted ? '4px solid var(--status-pending-dot)' : '1px solid var(--border)', borderRadius: '16px', padding: '16px', marginBottom: '10px', boxShadow: 'var(--shadow-card)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
        ...
        {showDate && (
          <span style={{ fontSize: '14px', color: 'var(--fg-2)', fontWeight: 600 }}>
            {new Date(b.date + 'T00:00:00').getMonth()+1}/{new Date(b.date + 'T00:00:00').getDate()}（{DAY_NAMES[new Date(b.date + 'T00:00:00').getDay()]}）
          </span>
        )}
      ...
      {b.status === 'pending' && (
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <button onClick={() => updateStatus(b.id, 'confirmed')} disabled={actionLoading === b.id}
            style={{ flex: 1, padding: '12px', fontSize: '16px', fontWeight: 700, background: 'var(--status-ok-bg)', color: 'var(--status-ok-fg)', border: '2px solid var(--status-ok-bd)', borderRadius: '10px', cursor: actionLoading === b.id ? 'wait' : 'pointer', fontFamily: 'inherit', minHeight: 'unset' }}>
            承認する
          </button>
          <button onClick={() => updateStatus(b.id, 'rejected')} disabled={actionLoading === b.id}
            style={{ flex: 1, padding: '12px', fontSize: '16px', fontWeight: 700, background: 'var(--status-full-bg)', color: 'var(--status-full-fg)', border: '2px solid var(--status-full-bd)', borderRadius: '10px', cursor: actionLoading === b.id ? 'wait' : 'pointer', fontFamily: 'inherit', minHeight: 'unset' }}>
            お断り
          </button>
        </div>
      )}
      ...
      {b.status === 'confirmed' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          {b.tel ? (
            <button onClick={() => handleCall(b)} disabled={actionLoading === b.id}
              style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 24px', fontSize: '17px', fontWeight: 700, background: 'var(--ocean)', color: '#fff', border: 'none', borderRadius: '10px', cursor: actionLoading === b.id ? 'wait' : 'pointer', fontFamily: 'inherit', minHeight: 'unset' }}>
