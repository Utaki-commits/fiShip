  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: v } = await supabase
        .from('vessels').select('id, name').eq('user_id', session.user.id).single()
      ...
      <div style={{ padding: '16px' }}>
        {done ? (
          <div style={{ background: 'var(--status-ok-bg)', border: '0.5px solid var(--status-ok-bd)', borderRadius: '14px', padding: '32px 20px', textAlign: 'center', marginTop: '20px' }}>
            ...
            <div style={{ fontSize: '22px', fontWeight: 500, color: 'var(--status-ok-fg)', marginBottom: '8px' }}>送信しました</div>
          ...
          <div style={{ background: '#FFFFFF', border: '0.5px solid #CDD3DC', borderRadius: '14px', padding: '18px', marginTop: '8px' }}>
            ...
            {error && (
              <div style={{ background: 'var(--status-full-bg)', border: '0.5px solid var(--status-full-bd)', borderRadius: '10px', padding: '12px 14px', marginBottom: '14px', fontSize: '16px', fontWeight: 500, color: 'var(--status-full-fg)' }}>
                {error}
              </div>
            )}
