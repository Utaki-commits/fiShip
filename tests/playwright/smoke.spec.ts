...
test.describe('public mobile smoke checks', () => {
  test('login page exposes captain login choices', async ({ page }) => {
    ...
    await expect(page.getByText('FiShip')).toBeVisible()
    await expect(page.getByRole('button', { name: /LINEでログイン/ })).toBeVisible()
    await expect(page.getByRole('button', { name: /電話番号でログイン/ })).toBeVisible()
  })

  test('legal terms page exposes the service rules', async ({ page }) => {
    await page.goto('/legal/terms')
    await expect(page.getByRole('heading', { name: '利用規約' })).toBeVisible()
    await expect(page.getByText('UTAKI SYSTEM')).toBeVisible()
  })
