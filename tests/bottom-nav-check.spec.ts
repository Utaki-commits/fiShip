...
const pages = [
  ...
  { name: 'extract', path: '/dashboard/extract' },
]

test('ボトムナビ選択状態確認', async ({ page }) => {
  fs.mkdirSync('docs/ai-reports/screenshots/bottom-nav-check', { recursive: true })
  ...
  if (TEST_EMAIL && TEST_PASSWORD) {
    ...
    test.skip(!SUPABASE_REF || !SUPABASE_URL, 'Supabase URL が未設定のためボトムナビをモック撮影できません')
    ...
    await page.route('**/rest/v1/bin_settings**', async route => {
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify([{ id: 'mock-bin-setting', vessel_id: 'bottom-nav-vessel', bin_type: 'day' }]),
      })
    })
    ...
    await page.route('**/rest/v1/passenger_logs**', async route => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) })
    })
  ...
  for (const target of pages) {
    ...
    await page.screenshot({
      path: `docs/ai-reports/screenshots/bottom-nav-check/${target.name}.png`,
      fullPage: true,
    })
  }
})
